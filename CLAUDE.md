...
- **이 세션 환경(샌드박스)의 아웃바운드 네트워크가 화이트리스트로 제한돼 있어서 `data4library.kr`을 직접 호출해서 실제 응답 스키마를 검증하지 못했다** — 공개 문서/커뮤니티 자료로 추정한 필드명(`response.docs[].doc.bookname/authors/bookImageURL` 등)으로 구현함. 실제 프로덕션(Vercel, 이 제한 없음)에서 응답이 다르게 오면 한 번 더 고쳐야 할 수 있음 — 사용자가 인증키 승인 후 실제 검색 결과를 확인해서 알려주면 그때 검증/수정
