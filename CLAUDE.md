...
- 사용자가 발급받은 인증키가 **"승인대기중"** 상태로 뜸 — 승인 전에는 호출이 실패(빈 배열 반환)할 수 있고, 승인되면 그때부터 실제로 검색에 반영됨. 승인 소요 시간은 문서에 명시돼 있지 않음
  - **업데이트(2026-09-18): 인증키 승인 완료.** 사용자가 실제 키 값을 전달해 `.env.local`의 `DATA4LIBRARY_AUTH_KEY`에 반영. 이 세션에서는 아웃바운드 네트워크가 열려 있어 `https://data4library.kr/api/srchBooks`를 직접 `curl`로 호출해 검증할 수 있었다("무지개 물고기" 검색 → `numFound: 24`, 응답 필드(`bookname`/`authors`/`isbn13`/`bookImageURL`)가 `src/lib/libraryBook.ts`가 기대하는 그대로 일치) — 지난 세션엔 샌드박스 네트워크 제한으로 매뉴얼 대조로만 검증했던 부분이 이제 실제 응답으로 확인됨. **Vercel Environment Variables(Production+Preview)에도 `DATA4LIBRARY_AUTH_KEY`를 등록해야 프로덕션 검색에도 반영된다** — 다른 키들과 같은 원칙대로 사용자가 직접 붙여넣을 것. 참고: Vercel은 환경변수만 바꾼다고 기존 배포에 자동 반영되지 않으므로, 등록 후 Redeploy(또는 새 커밋 push)가 필요하다.
...
## 연간 챌린지에 월별 진행 내역 추가 (2026-09-13)

사용자 요청 "배지 월별 달성도처럼 바꿔줘"가 정확히 무엇을 바꾸라는 건지 모호해서(연간 목표를 월간 목표로 바꾸라는 건지, 표시 스타일만 바꾸라는 건지, 월별 내역을 추가하라는 건지) `AskUserQuestion`으로 확인 — "연간 100권 목표는 그대로 두고, 월별로 몇 권씩 읽었는지 내역을 추가로 보여달라"는 뜻으로 확정됨.

- `src/lib/badges.ts`의 `YearlyChallenge`에 `monthlyCounts: number[]`(길이 12, index 0=1월) 추가 — `monthlyCountsForYear()`가 `recorded_at`에서 월을 뽑아 그 해 12개월 권수를 집계(다른 해 기록과 섞이지 않게 연도로 먼저 필터).
- `src/components/YearlyChallengeList.tsx`에 `MonthlyBreakdown` 서브컴포넌트 추가 — 연간 진행률 바 아래에 12개월 미니 막대그래프(달성 시 세이지 그린, 진행 중이면 오렌지, 기록 없는 달은 회색 점)를 표시. 자녀 홈(`/home`)과 부모 배지 비교(`/settings/badges`) 양쪽에 자동 반영(둘 다 이 컴포넌트를 공유하므로 별도 수정 불필요).
- `/settings/stats` 상단 요약 카드는 폭이 110px 안팎으로 매우 좁아 12개 막대를 넣기엔 무리라고 판단, 거기엔 손대지 않고 기존 "연간 N/100권" 텍스트만 유지.
- 테스트 3개 추가(월별 집계 정확성, 다른 해와 안 섞임, 빈 상태) — 전체 74개 통과. `npx tsc --noEmit`, `npm run build` 정상.
- 실제 브라우저로는 확인하지 않음(다개월치 기록 시딩 필요) — 이전 연간 챌린지 도입 때와 동일한 이유.

## 회원가입 화면에 "데모 체험하기" 링크 추가 (2026-09-15, 이후 로그인 화면으로 이동)

> **업데이트(같은 날)**: 처음엔 `/signup`에 넣었다가, 사용자가 "데모버튼을 회원가입이 아닌 첫 페이지 넣어줘"라고 요청 — 비로그인 방문자가 실제로 맨 처음 도착하는 화면은 `/`가 리다이렉트하는 `/login`이지 `/signup`이 아니었다(`src/app/page.tsx` 참고). 데모 카드+버튼을 `/signup`에서 `/login`으로 옮기고(로그인 폼 바로 위, 로고 아래) `/signup`은 원래의 가입 폼만 남김. 프로덕션(Vercel)에서도 실제 브라우저로 `/login`→데모 버튼→PIN→홈까지 재확인함.

사용자 요청: "리딩버디 회원가입 첫 화면에 데모 경험할 수 있는 링크를 제공하도록 개발해줘". 어떤 방식의 데모를 원하는지 `AskUserQuestion`으로 확인 — (1) **읽기 전용 데모 계정**(추천) 채택: 미리 만들어둔 데모 가족 계정에 한 번에 로그인시켜 실제 화면(대화/OCR/통계/배지 등)을 그대로 둘러보게 하되, 자녀 삭제·PIN 변경·기록 수정 같은 되돌릴 수 없는 쓰기는 막는다. (2) 데모 로그인 시 Azure OpenAI(대화 질문/감상문 생성) 실제 호출을 **허용**할지도 확인 — 세션당 호출이 적고 예산 내라 허용하기로 함(핵심 기능이라 실제로 체험해봐야 의미가 있다고 판단).

- **스키마**: `families.is_demo boolean not null default false`(`0012_demo_family.sql`) — 이번엔 SQL Editor 수동 실행 대신 **Supabase MCP(`apply_migration`)로 실제 프로젝트에 직접 적용**했다(기존 관행과 다른 점, 앞으로도 MCP가 연결돼 있으면 이 방법을 더 써도 될 듯). 기존 가족 2개(실제 데이터)는 전부 `false`로 안전하게 유지됨.
- **데모 계정 시딩**: `scripts/seed-demo-account.mjs`(`npm run seed:demo`) — `childAuth.ts`와 동일한 규칙(synthetic 이메일 + `CHILD_AUTH_SECRET` 기반 파생 비밀번호)으로 실제 계정을 만드는 멱등 스크립트. 매번 기존 `is_demo=true` 가족을 전부 지우고 새로 만들어서, 방문자가 대화 체험 등으로 데이터를 건드려도 다시 실행하면 깨끗해진다(실제 가족 데이터는 `is_demo` 조건으로 격리되어 절대 건드리지 않음). "체험 가족" 아래 부모 1명 + 자녀 2명(**봄이**: 배지 7종 전부 획득 가능하도록 대화 3건·OCR 3건·직접입력 5건=11권, 독서로 반영 5건, 이번 달 3권으로 다독왕까지 / **여름이**: 2권만 있는 대비군 — Phase 2 D 때와 같은 "형제자매 비교 보여주기" 패턴)을 시딩. 대화 기록 3건은 실제 `conversation_sessions`(4턴 메시지, `stage` 포함)까지 채워 넣어 "원본 대화 보기"도 정상 동작. PIN은 둘 다 `1234`(`demoMode.ts`의 `DEMO_CHILD_PIN`과 반드시 같아야 함 — 화면 안내 문구가 이 상수를 그대로 보여준다).
- **로그인 플로우**: `/signup` 최상단에 "🎈 가입 없이 먼저 둘러볼까요?" 카드 + "데모 체험하기" 버튼 → `POST /api/auth/demo-login`(서버 전용 `DEMO_PARENT_EMAIL`/`DEMO_PARENT_PASSWORD`로 `signInWithPassword`, 값은 클라이언트 코드에 절대 노출 안 됨) → `/profiles`. `/profiles`가 `is_demo`면 배너로 PIN을 직접 알려줘서(데모라 비밀로 감출 이유가 없음) 기존 PIN 화면 흐름을 그대로 타고 자녀 화면까지 들어갈 수 있다 — 로그인/PIN/세션전환 등 기존 인증 UX 자체도 자연스럽게 체험하게 되는 효과.
- **쓰기 차단 가드**: `src/lib/demoMode.ts`의 `isDemoFamily`/`demoBlockResponse`를 자녀 프로필 생성/수정/삭제/PIN재설정, 기록 수정, AI 코치 지침 저장, OCR 업로드/저장, 표지 사진 인식 라우트 맨 앞에 추가 — 데모 가족이면 403 + 안내 메시지를 돌려주고, 기존 폼들이 이미 갖고 있던 `error` 상태 UI가 그대로 그 메시지를 보여준다(새 UI를 거의 안 만들어도 됨). 다만 여러 단계를 거친 뒤에야 막혔다는 걸 알게 되는 막다른 흐름(자녀 등록/삭제/PIN재설정, OCR·표지인식 카드)은 버튼 자체를 미리 숨기거나 비활성화해서 사용자 경험을 다듬었다(`ChildEditCard`의 `readOnly` prop, `/profiles`·`/settings/children`의 "프로필 추가" 숨김, `/read/new`의 OCR 카드 비활성화, `NewBookForm`의 표지 인식 버튼 숨김).
- **대화/감상문 생성은 예외적으로 허용, 저장만 막음**: `next-question`/`essay` 라우트는 데모에서도 그대로 실제 Azure 호출을 한다. 다만 `reading-sessions/[id]/finish`는 데모 가족이면 `reading_records`를 실제로 만드는 대신 그 대화 세션만 정리(삭제, `conversation_sessions`에 RLS delete 정책이 없어 admin 클라이언트 사용 — `reading-sessions/[id]` DELETE 라우트와 동일한 이유)하고 `{ demo: true }`로 응답 — `ReviewSession.tsx`가 이 플래그를 보고 실제 저장 대신 "🎉 데모 체험은 여기까지예요!" 완료 카드를 보여준다. 방문자마다 다른 방문자가 만든 세션이 "이어서 쓰기"에 뜨는 것도 막기 위해 `/home`은 데모 가족이면 진행 중 세션 목록 자체를 비운다.
- **실제 브라우저(claude-in-chrome)로 전체 플로우 검증**: `/signup` → 데모 버튼 → `/profiles`(배너/PIN 안내) → 봄이 PIN 로그인 → `/home`(배지 7종 전부 획득, 연간 챌린지 11/100, 최근 기록 정상 표시, 데모 배너) → `/read/new`(OCR 카드 비활성화 확인) → 대화 시작(표지 인식 버튼 없음 확인) → 실제 4턴 AI 질문/답변 → 감상문 생성 → "저장" → 데모 완료 카드 확인 → DB로 실제 `reading_records`가 안 늘고 세션이 정리됐음을 SQL로 검증 → 재로그인해 부모 모드로 전환 → `/settings/children`(이름/PIN/삭제 전부 비활성화 확인) → `/settings/coach`에서 프리셋 선택 후 저장 시도 → 403 안내 메시지 확인까지 end-to-end로 확인함. `npx tsc --noEmit`(테스트 파일의 vitest 모듈 못 찾는 에러는 이 세션 이전부터 있던 환경 문제 — `node_modules`에 vitest 자체가 설치돼 있지 않음, `npm run test`도 동일한 이유로 커맨드를 못 찾음. 내가 건드린 소스 파일에는 타입 에러 없음), `npm run build`(31개 라우트 정상 컴파일) 확인.
- **로컬 개발 서버 포트 메모**: 이 세션에서 `localhost:3000`이 같은 머신의 다른 프로젝트(`voca_learning`, 같은 Supabase 프로젝트를 공유하는 `vocab_words` 등 테이블의 정체가 이걸로 확인됨)가 이미 점유하고 있어서, `.claude/launch.json`의 리딩버디 dev 서버 포트를 `3001`로 바꿔뒀다(포트 충돌 시 `npm run dev`에 `-p 3001`을 붙이는 식).
- **⚠️ 배포에 필요한 후속 조치**: `DEMO_PARENT_EMAIL`/`DEMO_PARENT_PASSWORD`(현재 `.env.local`에만 있음, 값은 이 세션이 생성)를 Vercel의 Environment Variables(Production + Preview)에도 등록해야 프로덕션에서 데모 버튼이 동작한다 — 다른 API 키들과 마찬가지로 **사용자가 직접 붙여넣을 것**. 또한 이 세션에서는 아직 `0012_demo_family.sql`을 실제 Supabase 프로젝트에 Supabase MCP로 직접 적용했으므로 SQL Editor에서 별도로 실행할 필요는 없다(이미 반영됨).
