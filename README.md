...
**앱 공식 명칭 '도란도란' 및 PWA 아이콘/브랜딩 확정 (2026-09-13)**: 기존 가칭 '리딩버디'에서, 아이들과 도란도란 이야기를 나누며 자연스럽게 남기는 독서 기록이라는 의미를 담아 **'도란도란(DoranDoran)'**으로 정식 명칭을 확정했다. 펼쳐진 책 위로 쌍둥이(세이지 그린/소프트 블루)를 상징하는 귀여운 말풍선이 떠오르는 공식 아이콘을 제작하여 `public/icons/`(PWA 192x192, 512x512, maskable, iOS apple-touch-icon) 및 브라우저 탭 파비콘(`favicon.ico`, `favicon-32x32.png`)으로 적용했다. 로그인 화면 헤더에도 공식 로고와 명칭을 반영했다.
