...
**Phase 2(MVP 이후 로드맵 중 A~D 선택)도 진행 중이다.** A(Vercel 배포)는 완료(위 "배포" 참고). B(독서 통계/리포트, `/settings/stats`)도 완료 — 자녀별 누적/이번 달 권수, 최근 6개월 추이 그래프, 기록 방식 비율, '독서로' 반영 현황을 보여준다(장르 분포는 스키마에 없어 미구현). 남은 건 C(표지 촬영 자동 인식), D(형제자매 배지·스탬프) — [CLAUDE.md](CLAUDE.md)의 체크리스트 참고.
