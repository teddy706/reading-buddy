...
**앱 공식 명칭 '리딩버디' 확정 및 PWA 아이콘/브랜딩 적용 (2026-09-13)**: 기존 프로젝트 가칭이었던 **'리딩버디(Reading Buddy)'**를 앱의 정식 명칭으로 최종 확정했다(타 서비스 명칭 중복 방지). 펼쳐진 책 위로 쌍둥이(세이지 그린/소프트 블루)를 상징하는 귀여운 말풍선이 떠오르는 공식 아이콘을 제작하여 `public/icons/`(PWA 192x192, 512x512, maskable, iOS apple-touch-icon) 및 브라우저 탭 파비콘(`favicon.ico`, `favicon-32x32.png`)으로 적용했다. 로그인 화면 헤더에도 공식 로고와 명칭을 반영했다.
