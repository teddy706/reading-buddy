...
| 테스트 | Vitest 2.1.9 | 순수 함수 단위 테스트 63개 (`npm run test`, §9 참고) |
...
src/
  ...
  lib/
    ...
    isbn.ts                     # ISBN 파싱(검색 API 응답)/검증(사람 입력)
...
supabase/migrations/0001~0011_*.sql
...
- 모바일(480px 이하)은 원래 고정폭 그대로다. `.app-shell` 하나가 모든 화면에서 쓰이며, 태블릿(`md`)·PC(`lg`)에서는 `md:max-w-2xl`, `lg:max-w-5xl`로 단계적으로 넓어진다(처음엔 부모 전용 화면에만 별도 클래스로 넣었다가, 결국 전 화면에 적용하면서 `.app-shell` 하나로 합쳤다 — CLAUDE.md "전 화면 태블릿/PC 반응형 대응" 참고).
- 화면 성격에 따라 두 갈래로 처리한다: (1) `/settings/**`, `/profiles` 등 **목록/그리드형 화면**은 셸이 넓어지는 만큼 내용도 여러 열로 배치(메뉴·자녀 카드·기록 카드를 `md`~`lg`부터 그리드로). (2) 로그인/PIN/대화(`ChatSession`)/감상문 검수(`ReviewSession`)/`NewBookForm`/`OcrReview`/`RecordDetail`/`/settings/coach`처럼 **폼이나 텍스트 위주 화면**은 셸은 넓히되 내용을 `mx-auto w-full max-w-*`(예: `max-w-sm`/`max-w-md`/`max-w-xl`) 래퍼로 감싸 폭을 제한한다 — 로그인 입력창이나 채팅 메시지, 감상문 문단이 화면 끝까지 늘어지면 오히려 읽기/쓰기 불편하기 때문. `flex-1`로 남은 세로 공간을 스크롤 영역이 차지해야 하는 화면(`ChatSession`, `ReviewSession`)은 이 래퍼도 `flex flex-1 flex-col`로 만들어야 세로 레이아웃이 유지된다.
- `RecordsBrowser`(기록 목록)는 자녀 본인 화면(`/records`)과 부모 화면(`/settings/records`)이 공유하는 컴포넌트라, `layout?: "list" | "grid"` prop(기본값 `"list"`)으로 넓은 배치를 옵트인시켰다 — 지금은 두 화면 다 `layout="grid"`를 넘겨 `md:grid-cols-2 lg:grid-cols-3` 배치를 쓴다. `list`는 그리드 배치가 필요 없는 다른 재사용처를 위한 기본값으로 남겨둠.
- **공유 컴포넌트를 넓힐 때는 재사용처를 먼저 확인할 것** — `BadgeGrid`(배지 그리드)는 부모의 `/settings/badges`뿐 아니라 자녀 홈(`/home`)에서도 재사용되고 있어서, 컬럼 수를 반응형으로 바꾸려던 시도를 되돌렸다(자녀 화면까지 새어나갈 뻔함). 이 화면은 원래도 3열 그리드가 자체적으로 충분히 유연해 손대지 않았다.
...
- **총 63개 테스트(10개 파일)**, `npm run test` (watch는 `npm run test:watch`) — `coachPresets.ts`(프리셋 뼈대 유지 검증), `bookSearch.ts`(다중 소스 중복 제거), `libraryBook.ts`(저자 필드 정리), `RecordsBrowser.tsx`의 `toIlikePattern`(ilike 검색어 이스케이프), `CoachSettingsForm.tsx`의 `matchingPresetId`(프리셋 일치 판정)이 최초 31개 이후 추가됨.
- **알아둘 점**:
